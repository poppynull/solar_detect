...
def test_one_img(img0):
    ...
    # data = ROOT / 'data/sun.yaml'  # dataset.yaml path
    ...
    for i, det in enumerate(pred):  # per image
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                # save_one_box(xyxy, small_img, file=save_dir / 'crops' / str(seen) / f'{p.stem}.jpg', BGR=True)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # save_one_box(xyxy, small_img, file=save_dir / 'crops' / str(seen) / f'{p.stem}.jpg', BGR=True)
                        annotator.box_label(xyxy, label, color=colors(c, True))
                cv2.putText(im0, str(len(det)), (20, 20), cv2.LINE_AA, 0.6, (0, 255, 0), 2)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'pt文件/best_300200.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'datasets/img/', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT / 'data/sun.yaml', help='(optional) dataset.yaml path')
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[320,200], help='inference size h,w')
